...
#a more direct account posits
#opinion should be driven by CR movement
#so plot this, instead. picking three points

#load genconventional
setwd(codedir); dir()
source('genconventional.R')
tmpdims<-c("anxiety","punitive")
tmpdf<-lapply(tmpdims,function(dim) {
  tmpdf<-genconventional(25/100,60/100,47.5/100)
  tmpdf$mu<-tmpdf$y
  tmpdf$y<-NULL
  tmpdf$state_alpha2<-"all"
  tmpdf$race<-1
  tmpdf$facet<-"conventional" 
  tmpdf$dimension<-dim
  tmpdf$hypothesis<-"cr"
...
plotdfs[['conventional_loess']]<-tmpdf

tmpdims<-c("anxiety","punitive")
tmpdf<-lapply(tmpdims,function(dim) {
  tmpdf<-genconventional(25/100,60/100,60/100)
  tmpdf$mu<-tmpdf$y
  tmpdf$y<-NULL
  tmpdf$state_alpha2<-"all"
  tmpdf$race<-1
  tmpdf$facet<-"conventional" 
  tmpdf$dimension<-dim
  tmpdf$hypothesis<-"cr_alt"
  tmpdf
}) %>% rbind.fill
plotdfs[['conventional_loess_alt']]<-tmpdf
