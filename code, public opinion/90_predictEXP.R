...
fulloutput<-lapply(tmpseq.i,function(i) {
  ...
  predictdf$divisionXyear<-paste0(
    predictdf$division,"_",predictdf$year
  )
  predictdf$edXyear<-paste0(
    predictdf$ed,"_",predictdf$year
  )
  #add others
...
pstratdf<-lapply(tmpseq.i,function(i) {
  ...
  #add white south/non-south: for revanchist argument
  predictdf$south<-predictdf$division%in%c(5,6,7)
  thisdf2<-expand.grid(
    year=c(
      unique(predictdf$year)
    ),
    south=c(
      unique(predictdf$south)
    ),
    ed=c(1,4),
    race=1,
    stringsAsFactors=F
  )
  #add blacklowed/blackhighed: for respectability politics
  thisdf3<-expand.grid(
    year=c(
      unique(predictdf$year)
    ),
  ...
  thisdf<-lapply(thisdf,function(x) {
    tmp<-is.na(x)
    x[tmp]<-"all"
    x
  }) %>% as.data.frame
